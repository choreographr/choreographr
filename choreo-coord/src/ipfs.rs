...
pub fn add(bytes: &[u8], filename: &str) -> Result<String, CoordError> {
    ...
    let response = ureq::post(&format!("{IPFS_API_URL}/api/v0/add"))
        ...
        .query("quieter", "true")
        .send(form)
        .map_err(|e| CoordError::Ipfs(format!("failed to upload payload: {e}")))?;

    let text = response
        .into_body()
        .read_to_string()
        .map_err(|e| CoordError::Ipfs(format!("failed to read add response: {e}")))?;
...
/// CID for `api/v0/cat`.
pub fn cat(digest_hex: &str) -> Result<Vec<u8>, CoordError> {
...
pub fn cat_by_cid(cid: &str) -> Result<Vec<u8>, CoordError> {
    ...
    response
        ...
        .into_reader()
        .read_to_end(&mut buf)
        .map_err(|e| CoordError::Ipfs(format!("failed to read ipfs bytes for {cid}: {e}")))?;
    Ok(buf)
}
...
pub fn id() -> Result<IpfsPeerInfo, CoordError> {
    ...
    let bytes = response
        .into_body()
        .read_to_string()
        .map_err(|e| CoordError::Ipfs(format!("failed to read /id response: {e}")))?;
    let info: IpfsPeerInfo = serde_json::from_str(&bytes)
        .map_err(|e| CoordError::Ipfs(format!("failed to decode /id response: {e}")))?;
    Ok(info)
}

/// Convert a digest hex to the raw 32-byte digest (validates the length).
pub fn digest_bytes(digest_hex: &str) -> Result<[u8; 32], CoordError> {
