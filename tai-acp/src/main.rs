...
#[derive(Parser)]
#[command(name = "tai-acp", about = "Agent Client Protocol (ACP) bridge for tai")]
struct Cli {
    /// Path to the tai-daemon Unix socket.
    #[arg(long = "socket-path", default_value_t = tai_proto::socket_path())]
    socket_path: String,

    /// Path to the log file (stderr is unused to avoid corrupting the ACP protocol stream).
    #[arg(long = "log-file", default_value_t = String::from("/tmp/tai-acp.log"))]
    log_file: String,
}
...
fn setup_logging(log_file: &str) -> Result<(), anyhow::Error> {
    ...
    let file_layer = tracing_subscriber::fmt::layer()
        ...
        .with_ansi(false);
    let tai_acp_directive = "tai_acp=debug".parse().unwrap_or_else(|e| {
    ...
    let filter = tracing_subscriber::EnvFilter::builder()
        .from_env_lossy()
        .add_directive(tai_acp_directive);
...
fn main() -> Result<(), anyhow::Error> {
    ...
    // Logging goes to /tmp/tai-acp.log (never stderr, which is unused
    ...
    let (event_tx, event_rx) = mpsc::channel::<tai_acp::daemon_client::Event>();
    ...
    let (daemon_client, writer_handle) =
        tai_acp::daemon_client::spawn_daemon_io(&cli.socket_path, event_tx.clone())
            .with_context(|| format!("could not connect to tai-daemon at '{}'", cli.socket_path))?;
    handles.push(writer_handle);
    handles.push(daemon_client.join_handle);

    // 2. Spawn the ACP stdin reader thread.
    let reader_handle = tai_acp::acp_reader::spawn_acp_reader(event_tx.clone())
    ...
    if let Err(e) = tai_acp::acp_handler::run_event_loop(event_rx, daemon_client.writer_tx) {
    ...
    tracing::info!("tai-acp shutting down");
