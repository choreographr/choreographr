//! Page/overlay state: the `Page` enum, the AI-providers page
//! (`AIProvidersState`, `AccountWizardState`, `CredentialModalState`), the
//! Polkadot-account import modal (`PolkadotImportState`), and the
//! model-selector popup (`ModelSelectorState`), plus the page-layout constants
//! they share.  The session-manager list/detail state lives in the sibling
//! `state/session_manager.rs`; the history marker (`Marker`) lives here.

use super::{InputBuffer, ProviderInfo};
