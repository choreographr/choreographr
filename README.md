...
Choreographr is an all-purpose extensible AI agent system written entirely in Rust. It has a client/server architecture and can run many sessions simultaneously. It can be run locally or in the cloud. LLM generated code can be run in a sandboxed RISC-V VM for complete security and observability.

### All Purpose

Choreographr is an all-purpose agent. It can be used for software development, a personal / business agent, or as a research tool. It can run on your desktop or in the cloud.

## Documentation

Full, always-current documentation — installation, configuration, the tool
reference, and the architecture deep dive — lives on the website at
**[choreographr.com](https://choreographr.com/agent/docs/)**. This README covers
the essentials; the website is the reference for the details.
...
the derived public binding, created once via the `BindKeystore` wire path —
TOFU-once, no rotation). A client holds one unlock key per daemon in
`known_servers.toml`; the binding key is freshly generated by the client at
auto-bind time and recorded before the bind is sent. Pre-held keys (a stored
per-daemon key, or the legacy raw `identity.pk` file, which is copied into
the store on first use and never deleted) only VERIFY existing bindings —
they never create one. The daemon starts locked and only decrypts
...
- **GitHub Releases tarball** — download
  `choreographr-<version>-aarch64-apple-darwin.tar.gz` (Apple Silicon) or
  `choreographr-<version>-x86_64-apple-darwin.tar.gz` (Intel — built for
...
The daemon is a user service that **you** start — installers place
...
Requires a [Rust toolchain](https://rustup.rs/) — minimum supported Rust version (MSRV) is **1.95** — and a [Zig toolchain](https://ziglang.org/) (`brew install zig`), which `choreographr` needs to compile the `zlob` glob/walker dependency.
...
The **source uses no nightly-only features**, so the code also builds on any stable ≥ 1.95 (the MSRV floor, enforced by CI). Stable builds are a supported but explicit opt-out, because the nightly-only flags we wire in hard-block stable *Cargo* — use the `just` recipes, which temporarily strip those flags for one command and restore them:
...
database. Each daemon's keystore is bound to one unlock key held client-side;
the binding is created once via the `BindKeystore` wire path (TOFU-once), and
...
- `/unlock <base64 unlock-key>` — unlock with the given 32-byte key (base64); it is recorded into `known_servers.toml` only after the daemon confirms the unlock. There is no `/bind-key` command — a fresh (unbound) daemon is auto-bound on connect with a newly generated key
...
- `Alt+R` — cycle reasoning effort through available slugs for the attached session's model (status message states: model reports no effort levels → "model does not support reasoning"; no model selected → "no model selected — pick one with Alt+M"; model selected but capability not yet reported → "reasoning capability not yet available")
- `Alt+M` — open the model selector: list models available on the attached session's account, type to filter, `↑`/`↓`/`PgUp`/`PgDn` or the mouse wheel to navigate, click a row or press Enter to select, Esc to dismiss
- `Alt+Up` / `Alt+Down` — undo / redo the most recent user turn (same as `/undo` and `/redo`)
- `Alt+H` — toggle the keyboard-shortcut help overlay (the two cyan lines at the bottom of the Chat page)
- `Alt+Q` — quit the client from any page or open modal (same as `/quit`)
- `Ctrl+Backspace` — clear the draft prompt in the input box (empties the whole draft wherever the cursor sits; `Ctrl+W` deletes the previous word and `Ctrl+U` clears only up to the cursor; inert while browsing prompt history with `↑`, where it can neither clear nor detach the recalled entry)
- **Readline / emacs editing keys** — the prompt honours the shell's readline bindings. Movement: `Ctrl+A`/`Ctrl+E` (start/end of the logical line), `Ctrl+B`/`Ctrl+F` (one char), `Alt+B`/`Alt+F` (one word), `Home`/`End` (line edges), `Ctrl+Home`/`Ctrl+End` (buffer edges). Killing/editing: `Ctrl+D` (delete char), `Ctrl+H`/`Backspace` (delete char back), `Ctrl+K` (kill to end of line), `Ctrl+T` (transpose), `Ctrl+W` (kill word back), `Alt+D` (kill word forward), `Alt+Backspace` (kill word back), `Ctrl+U` (kill to line start). History: `Ctrl+P`/`Ctrl+N` (previous/next prompt, the same as `↑`/`↓`). Every app *command* lives on an `Alt+` chord (above), so these `Ctrl+`/`Alt` editing chords are free
...
match. `AddCredential` also carries the unlock key and implicitly unlocks on
a valid blob. Keys are recorded into `known_servers.toml` only on the
daemon's targeted confirmation (`Unlocked` / `Bound` / `CredentialAdded`);
nothing is ever auto-deleted (`KnownServers::remove(addr)` re-pair is the
only removal path).

**Multi-client provisioning:** the first client to connect to a fresh daemon
binds it with a minted key; other clients get `LockedError` until the key is
shared out-of-band (add `unlock_key` to their `known_servers.toml` entry, or
use `/unlock <base64-key>`). Resetting the daemon's database invalidates its
binding; the next connect auto-binds a fresh key.
...
[Build from source](#build-from-source) section (cargo ≥ 1.95 + zig), and
...
just pre-commit           # the commit gate: clippy-strict + test-all + fmt
just pre-release          # the release gate: preflight + release-state + fmt-check + clippy-strict + test-all + the supply-chain guard + crates.io token check, then push master + dispatch the release-workflow dry run
just release-notes X.Y.Z  # preview the release notes, generated from commit messages by git-cliff
just check-release-state  # release preflight: on master, clean, not behind origin/master
just check-crates-io-token  # confirm the stored crates.io token authenticates (release preflight)
just release-workflow-dry-run  # push master + kick the release workflow (workflow_dispatch; builds all platforms, creates no release)
