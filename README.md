...
- Mouse select-to-copy — drag to select text in the history pane; on release it is copied to the system clipboard automatically (via the OSC 52 escape sequence, so it works in kitty/wezterm/ghostty/alacritty ≥0.13/Windows Terminal and over SSH/tmux, and is a silent no-op in terminals without OSC 52 support such as macOS Terminal.app), and the status line reports "N tokens copied to clipboard". Selection starts only on plain text — clicking a reasoning/tool-result header still toggles it, and a plain click without a drag copies nothing
