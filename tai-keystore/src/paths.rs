...
thread_local! {
    ...
    /// a tai-daemon subdirectory inside this path instead of the real
...
/// When `root` is `Some(path)`, `config_dir()` returns `path/tai-daemon`.
#[doc(hidden)]
...
/// Returns the config directory (`{config}/tai-daemon`).
///
...
mod tests {
    ...
    fn test_override_used_when_set() {
        let temp = std::env::temp_dir().join("tai-keystore-test-override");
