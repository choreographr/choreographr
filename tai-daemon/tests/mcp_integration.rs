...
/// execution through the full tai-daemon stack (McpManager + ToolRegistry).
///
...
fn mcp_server_everything_tools_are_discovered_and_callable() {
    ...
    let config_path = config_dir.path().join("tai-daemon");
    std::fs::create_dir_all(&config_path).expect("create tai-daemon config dir");
    ...
    let mut registry = tai_daemon::tools::ToolRegistry::new();
    let mcp_manager = tai_daemon::mcp::McpManager::from_config(&mut registry);
