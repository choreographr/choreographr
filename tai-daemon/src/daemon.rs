...
impl DaemonState {
    pub fn handle_command(&mut self, cmd: DaemonCommand) {
        match cmd {
            DaemonCommand::CreateSession { title, parent_session_id, cwd, max_turns, reply } => {
                ...
                let handle = task::spawn_blocking(move || {
                    session_main(
                        session_rx, sid, db, client, tool_registry,
                        daemon_tx, Some(init_record), max_turns_default,
                    );
                });

                self.active_sessions.insert(sid, ActiveSessionEntry {
                    cmd_tx: session_tx.clone(),
                    handle,
                });
                self.session_metadata.insert(sid, SessionMetadata {
                    title: title.clone(),
                    selected_model: None,
                    parent_session_id,
                    ...
                    None => {
                        match db::read_session(&self.db, session_id) {
                            Ok(Some(record)) => {
                                ...
                                let handle = task::spawn_blocking(move || {
                                    session_main(
                                        session_rx, session_id, db, client, tool_registry,
                                        daemon_tx, Some(record), max_turns_default,
                                    );
                                });

                                self.active_sessions.insert(session_id, ActiveSessionEntry {
                                    cmd_tx: session_tx.clone(),
                                    handle,
                                });
                            ...
                            Ok(None) => {
                                let _ = reply.send(Err(io::Error::new(io::ErrorKind::NotFound, "session not found")));
                            }
                            Err(e) => {
            ...
            DaemonCommand::ListSessions { reply } => {
                let mut summaries: Vec<SessionSummary> = self.session_metadata.iter()
            ...
            DaemonCommand::GetSession { session_id, reply } => {
                let summary = self.session_metadata.get(&session_id).map(|meta| SessionSummary {
                    session_id,
                    title: meta.title.clone(),
                    selected_model: meta.selected_model.clone(),
                    parent_session_id: meta.parent_session_id,
            ...
            DaemonCommand::UpdateMetadata { session_id, metadata } => {
...
fn handle_unlock_inner(state: &mut DaemonState, passphrase: String) -> Result<(), String> {
    ...
    match keystore.get_api_key("openai") {
        Some(api_key) => {
            let service_config = crate::openai::load_service_config()
                .unwrap_or_default();
        ...
        None => {
            Err("no 'openai' credential found in keystore".to_string())
        }
    }
}
...
fn handle_list_models_inner(
    ...
    let client = state.openai_client.as_ref()
        .ok_or("daemon is locked".to_string())?;
    let models = client.validate_and_list_models()
