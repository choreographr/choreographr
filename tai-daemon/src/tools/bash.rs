...
define_tool_with_cwd!(
    Bash,
    "bash",
    "Execute a shell command in the project directory. Returns combined stdout/stderr and exit code. Non-interactive only — commands that read from stdin will hang.",
    execute_bash_tool,
    serde_json::json!({
        "type": "object",
        "properties": {
...
pub fn execute_bash_tool(arguments_json: &str, cwd: Option<&std::path::Path>) -> ToolResult {
    match execute_bash_inner(arguments_json, cwd) {
        Ok(content) => tool_ok(content),
        Err(error) => error.into(),
    }
}

fn execute_bash_inner(
    arguments_json: &str,
    cwd: Option<&Path>,
