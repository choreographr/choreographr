...
pub(crate) fn execute_evm_balance_tool(arguments_json: &str) -> ToolResult {
    match execute_evm_balance_inner(arguments_json) {
...
fn execute_evm_balance_inner(arguments_json: &str) -> Result<String, ToolError> {
    ...
    let output = tokio::runtime::Handle::current().block_on(evm_balance_impl(&args.rpc_url, &args.address))?;
