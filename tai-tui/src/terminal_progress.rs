...
fn build_seq(context_window: Option<u32>, token_usage: Option<&TokenUsage>) -> String {
    match context_window {
        Some(cw) if cw > 0 => match token_usage {
            Some(usage) => {
                // Use u64 for intermediate arithmetic to avoid any surprise
                // around u32::MAX * 100 overflowing.
                let pct = (usage.total_tokens as u64).saturating_mul(100) / cw as u64;
                let pct = pct.min(100);
...
/// - If `token_usage` is `None` → shows indeterminate progress (spinner).
/// - Otherwise → shows a percentage bar: `total_tokens / context_window`,
///   capped at 100%.
///
/// This is a no-op on terminals that don't support OSC 9;4.
pub fn update_terminal_progress(token_usage: Option<&TokenUsage>, context_window: Option<u32>) {
    if !supports_progress() {
        return;
    }

    let seq = build_seq(context_window, token_usage);
...
mod tests {
    ...
    #[test]
    fn indeterminate_when_no_token_usage() {
    ...
    // `build_seq` is the same function used by the production code above.
}
