...
- **Waits are event-driven**: use `select!`/`select_biased!` (with the cancel/shutdown arm biased first), a blocking `recv`, or `recv_deadline`. Never use `recv_timeout` as a poll interval paired with a flag check, and never a `sleep`-poll loop; a timer that *is* the event uses `crossbeam_channel::after(..)`.
