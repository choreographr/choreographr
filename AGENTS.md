...
7. **The provider-socket registry** (`choreo-sockreg`'s `SocketRegistry` — one daemon-wide handle, `Arc<Mutex<Vec<OwnedSock>>>`, created in `DaemonState::open` and shared with every `InferenceProvider` client and the cancel/suspend handlers). Its whole purpose is to reach into another thread's blocked provider `read()`: channels cannot interrupt a syscall, but `shutdown(fd, SHUT_RDWR)` makes the blocked read return immediately, so cancel and suspend events can un-block wedged inference workers. The mutex is held only for a handful of fd syscalls and carries no message traffic; the Arc handle itself is immutable plumbing (never mutated after creation) passed down construction sites, not a shared mutable channel replacement. The rationale lives in ARCHITECTURE.md's `choreo-sockreg` section.

### Channel selection (crossbeam vs std vs async)

Thread-to-thread messaging must use **`crossbeam_channel`**, not `std::sync::mpsc`, in the crates that already depend on it (`choreo-daemon`, `choreo-tui`, `choreo-client-core`, `choreo-ai-protocols`). Crossbeam is a strict superset for our needs: cloneable receivers (true MPMC), `select!`/`select_biased!` (including send arms and timer channels), blocking iterators, and a consistent error taxonomy. std `mpsc` cannot join a `select!` later or add a second receiver without a redesign, so defaulting to crossbeam keeps future evolution a one-line change. This is a consistency/future-proofing rule, not a performance rule — our channels are control-plane and human-rate, so channel throughput is never the bottleneck (see also the crossfire evaluation: crossbeam stays; crossfire is rejected codebase-wide).

Apply it as follows:

- **New code** in the four crates above: always `crossbeam_channel`.
- **Existing std `mpsc`**: convert opportunistically, when the change being made would otherwise strain single-consumer semantics (session event streams, per-request replies that might get tapped by metrics/tracing later), or the moment the channel needs `select!`, a cloned receiver, or a select-with-timeout. Do not migrate one-shot reply/flag channels or the existing test-site constructions for their own sake — that is churn with no behavioral payoff.
- **Leaf crates** (`choreo-acp`, `choreo-gui`, `choreo-im`, `choreo-mcp`, `choreo-transport`): std `mpsc` is fine for their trivial single-consumer fan-in/out; do not add a crossbeam dependency just for consistency. If such a crate grows the needs above, add `crossbeam-channel.workspace = true` at that point.
