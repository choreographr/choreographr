/// Integration test for the tai-mcp crate.
///
...
fn mcp_server_everything_can_be_spawned_and_tools_listed() {
    let config = tai_mcp::McpServerConfig {
    ...
    let mut client = tai_mcp::McpClient::spawn(&config).expect("spawn MCP server");
    ...
    let result = client
        .call_tool(
            "echo",
            Some(serde_json::json!({"message": "hello from tai"})),
            None,
        )
    ...
    assert!(
        result.content.iter().any(|c| {
            matches!(
                c,
                tai_mcp::McpContent::Text { text } if text.contains("hello from tai")
            )
        }),
