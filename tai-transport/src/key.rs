...
/// Path to the Noise IK static secret key (~/.config/tai/transport.sec)
pub fn transport_sec_path() -> Result<PathBuf, TransportError> {
    let config = dirs::config_dir().ok_or(TransportError::ConfigDirNotFound)?;
    Ok(config.join("tai").join("transport.sec"))
}

/// Path to the Noise IK static public key (~/.config/tai/transport.pub)
pub fn transport_pub_path() -> Result<PathBuf, TransportError> {
    let config = dirs::config_dir().ok_or(TransportError::ConfigDirNotFound)?;
    Ok(config.join("tai").join("transport.pub"))
}
...
/// (~/.config/tai/transport.sec and ~/.config/tai/transport.pub).
///
...
mod tests {
    ...
    fn ensure_transport_keypair_generates_and_loads() {
        ...
        assert!(temp.path().join("tai").join("transport.sec").exists());
        assert!(temp.path().join("tai").join("transport.pub").exists());
    }
}
