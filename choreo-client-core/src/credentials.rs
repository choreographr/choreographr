...
/// 2. Reading `identity.pk.enc` + decrypting with `CHOREOGRAPHR_KEYSTORE_PASSPHRASE` env var —
...
pub fn read_public_key_bytes() -> Result<[u8; 32], ClientError> {
    ...
    let path = choreo_keystore::paths::public_key_path()
...
mod tests {
    ...
    fn try_auto_unlock_key_with_encrypted_key() {
        ...
        let old = std::env::var("CHOREOGRAPHR_KEYSTORE_PASSPHRASE").ok();
        // SAFETY: single-threaded test context; no other test reads CHOREOGRAPHR_KEYSTORE_PASSPHRASE.
        unsafe { std::env::set_var("CHOREOGRAPHR_KEYSTORE_PASSPHRASE", "hunter2") };
        let result = try_auto_unlock_key();
        // Restore the previous value (if any) or remove.
        // SAFETY: same single-threaded test context.
        match old {
            Some(v) => unsafe { std::env::set_var("CHOREOGRAPHR_KEYSTORE_PASSPHRASE", v) },
            None => unsafe { std::env::remove_var("CHOREOGRAPHR_KEYSTORE_PASSPHRASE") },
        }
