...
#!/bin/bash
# Generated by scripts/check-ios.sh — do not edit (see build-ios.sh).
# Translate clang/rust-style triples to zig's form (see build-ios.sh for the
# full rationale: build scripts in the choreo-daemon tree both COMPILE for
# Apple targets and LINK dylibs; every Apple-iOS invocation — compile or
# link — keeps the macOS rewrite, since zig bundles macOS libc/stubs only).
