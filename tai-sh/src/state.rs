...
impl App {
    ...
    pub(crate) fn append_stream_text(&mut self, request_id: u32, stream: OutputStream, chunk: &str) {
        if !self.in_progress.contains_key(&request_id) {
    ...
    pub(crate) fn scroll_down(&mut self, amount: usize) {
        self.history_scroll.scroll_down(amount, self.max_scroll_offset());
    }
...
pub(crate) fn session_message_lines(message: &SessionMessage, width: u16) -> Vec<String> {
    match message {
        ...
        SessionMessage::ToolResult { name, content, is_error, .. } => {
...
fn render_markdown_blocks(blocks: &[MarkdownBlock], lines: &mut Vec<String>, indent: usize, width: usize) {
...
fn render_markdown_block(block: &MarkdownBlock, lines: &mut Vec<String>, indent: usize, width: usize) {
    match block {
        ...
        MarkdownBlock::CodeBlock { language, code } => {
            let header = language.as_deref().map(|value| format!("```{value}")).unwrap_or_else(|| "```".to_string());
        ...
        MarkdownBlock::BlockQuote(blocks) => {
            ...
            for line in quoted { lines.push(indented_line(indent, format!("> {line}"))); }
        }
        MarkdownBlock::List { ordered, start, items } => {
            for (index, item) in items.iter().enumerate() {
                ...
                for line in rendered_iter { lines.push(format!("{}{}", " ".repeat(continuation_indent), line)); }
            }
        }
        MarkdownBlock::Table { alignments, header, rows } => lines.extend(render_table_lines(alignments, header, rows, indent, width)),
...
fn render_table_lines(
    ...
    let column_count = alignments.len().max(header.len()).max(rows.iter().map(Vec::len).max().unwrap_or(0));
    if column_count == 0 { return vec![String::new()]; }
    let mut table_rows = Vec::with_capacity(rows.len() + 1);
    table_rows.push(normalize_table_row(header, column_count));
    table_rows.extend(rows.iter().map(|row| normalize_table_row(row, column_count)));
    let mut widths = vec![3usize; column_count];
    for row in &table_rows { for (index, cell) in row.iter().enumerate() { for line in cell.lines() { widths[index] = widths[index].max(display_width(line)); } } }
    let border_width = column_count * 3 + 1;
    let available = width.saturating_sub(indent).max(border_width + column_count);
    ...
    lines.extend(render_table_row_wrapped(&table_rows[0], &widths, &header_alignment, indent));
    lines.push(table_separator_line(&widths, &header_alignment, indent));
    for (index, row) in table_rows.iter().enumerate().skip(1) {
        lines.extend(render_table_row_wrapped(row, &widths, &header_alignment, indent));
        if index < table_rows.len() - 1 { lines.push(table_border_line('├', '┼', '┤', &widths, indent)); }
    }
    lines.push(table_border_line('└', '┴', '┘', &widths, indent));
    lines
}

fn normalized_alignments(alignments: &[MarkdownAlignment], column_count: usize) -> Vec<MarkdownAlignment> {
    (0..column_count).map(|index| alignments.get(index).copied().unwrap_or(MarkdownAlignment::None)).collect()
}

fn normalize_table_row(row: &[Vec<MarkdownInline>], column_count: usize) -> Vec<String> {
    (0..column_count).map(|index| row.get(index).map(|cell| inline_plain_text(cell)).unwrap_or_default()).collect()
}

fn shrink_column_widths(widths: &mut [usize], budget: usize) {
    let min_width = 3usize;
    while widths.iter().sum::<usize>() > budget {
        if let Some((index, _)) = widths.iter().enumerate().filter(|(_, width)| **width > min_width).max_by_key(|(_, width)| **width) {
            widths[index] -= 1;
        } else { break; }
    }
}

fn table_border_line(left: char, middle: char, right: char, widths: &[usize], indent: usize) -> String {
    let mut text = String::new();
    text.push(left);
    for (index, width) in widths.iter().enumerate() {
        text.push_str(&"─".repeat(*width + 2));
        text.push(if index + 1 == widths.len() { right } else { middle });
    }
    indented_line(indent, text)
}

fn table_separator_line(widths: &[usize], alignments: &[MarkdownAlignment], indent: usize) -> String {
    let mut text = String::new(); text.push('├');
    for (index, width) in widths.iter().enumerate() {
        text.push_str(&alignment_rule_segment(*width, alignments[index]));
        text.push(if index + 1 == widths.len() { '┤' } else { '┼' });
    }
    indented_line(indent, text)
}
...
fn alignment_rule_segment(width: usize, alignment: MarkdownAlignment) -> String {
    ...
    match alignment {
        ...
        MarkdownAlignment::Center => if inner <= 2 { ":".repeat(inner) } else { format!(":{}:", "─".repeat(inner - 2)) },
        MarkdownAlignment::Right => format!("{}:", "─".repeat(inner.saturating_sub(1))),
        MarkdownAlignment::None => "─".repeat(inner),
    }
}

fn render_table_row_wrapped(row: &[String], widths: &[usize], alignments: &[MarkdownAlignment], indent: usize) -> Vec<String> {
    let wrapped_cells: Vec<Vec<String>> = row.iter().zip(widths.iter()).map(|(cell, width)| wrap_cell_text(cell, *width)).collect();
    ...
    for line_index in 0..row_height {
        ...
        for column_index in 0..widths.len() {
            let cell_line = wrapped_cells[column_index].get(line_index).map(String::as_str).unwrap_or("");
            text.push(' ');
            text.push_str(&pad_aligned(cell_line, widths[column_index], alignments[column_index]));
            text.push(' '); text.push('│');
        }
...
fn wrap_cell_text(text: &str, width: usize) -> Vec<String> {
    let width = width.max(1); let mut lines = Vec::new();
    for raw_line in text.split('\n') {
        let mut current = String::new(); let mut current_width = 0;
        for word in raw_line.split_whitespace() {
            let word_width = display_width(word); let separator_width = usize::from(!current.is_empty());
            if current_width + separator_width + word_width <= width {
                if separator_width == 1 { current.push(' '); current_width += 1; }
                current.push_str(word); current_width += word_width;
            } else if current.is_empty() {
                lines.extend(split_word_to_width(word, width));
            } else {
                lines.push(std::mem::take(&mut current)); current_width = 0;
                if word_width <= width { current.push_str(word); current_width = word_width; } else { lines.extend(split_word_to_width(word, width)); }
            }
        }
        if current.is_empty() { lines.push(String::new()); } else { lines.push(current); }
    }
...
fn split_word_to_width(word: &str, width: usize) -> Vec<String> {
    let width = width.max(1); let mut chunks = Vec::new(); let mut current = String::new(); let mut current_width = 0;
    for grapheme in unicode_segmentation::UnicodeSegmentation::graphemes(word, true) {
        ...
        if !current.is_empty() && current_width + grapheme_width > width { chunks.push(std::mem::take(&mut current)); current_width = 0; }
        current.push_str(grapheme); current_width += grapheme_width;
        if current_width >= width { chunks.push(std::mem::take(&mut current)); current_width = 0; }
    }
...
fn pad_aligned(text: &str, width: usize, alignment: MarkdownAlignment) -> String {
    ...
    if text_width >= width { return text.to_string(); }
...
fn grapheme_width(grapheme: &str) -> usize {
    if grapheme.is_empty() { 0 } else {
        unicode_width::UnicodeWidthStr::width(grapheme).max(
            grapheme.chars().map(|ch| unicode_width::UnicodeWidthChar::width(ch).unwrap_or(0)).max().unwrap_or(0)
        )
    }
}

fn inline_plain_text(inlines: &[MarkdownInline]) -> String {
    let mut text = String::new(); append_inline_plain_text(inlines, &mut text); text
}
...
fn append_inline_plain_text(inlines: &[MarkdownInline], text: &mut String) {
    for inline in inlines {
        match inline {
            ...
            MarkdownInline::Emphasis(content) | MarkdownInline::Strong(content) => append_inline_plain_text(content, text),
            MarkdownInline::Link { content, destination } => {
                append_inline_plain_text(content, text);
                if !destination.is_empty() { text.push_str(" ("); text.push_str(destination); text.push(')'); }
            }
            MarkdownInline::Image { alt, destination } => {
                text.push_str("[image: "); append_inline_plain_text(alt, text);
                if !destination.is_empty() { text.push_str("] ("); text.push_str(destination); text.push(')'); } else { text.push(']'); }
            }
...
fn indented_line(indent: usize, text: String) -> String {
    if indent > 0 { format!("{}{}", " ".repeat(indent), text) } else { text }
}

fn inlines_to_lines(inlines: &[MarkdownInline], indent: usize, prefix: Option<String>) -> Vec<String> {
    let mut lines = Vec::new(); let mut current = String::new();
    if indent > 0 { current.push_str(&" ".repeat(indent)); }
    if let Some(prefix) = prefix { current.push_str(&prefix); }
    render_inlines_to_lines(inlines, &mut lines, &mut current, indent);
    lines.push(current); lines
}

fn render_inlines_to_lines(inlines: &[MarkdownInline], lines: &mut Vec<String>, current: &mut String, indent: usize) {
    for inline in inlines {
        match inline {
            MarkdownInline::Text(text) | MarkdownInline::Code(text) => current.push_str(text),
            MarkdownInline::Emphasis(content) | MarkdownInline::Strong(content) => render_inlines_to_lines(content, lines, current, indent),
            MarkdownInline::Link { content, destination } => {
                render_inlines_to_lines(content, lines, current, indent); current.push_str(&format!(" ({destination})"));
            }
            MarkdownInline::Image { alt, destination } => {
                current.push_str("[image: "); render_inlines_to_lines(alt, lines, current, indent); current.push_str(&format!("] ({destination})"));
            }
...
pub(crate) fn image_block_height(available_height: usize) -> usize { available_height.min(12) }
