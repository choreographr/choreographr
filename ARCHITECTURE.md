...
| `images/` | **Image generation clients** — the `ImageGenerationClient` trait (`generate_image(req, cancel_rx)`, `provider_slug()` — NO default-model surface: model choice is purely catalog/tool-side, never a guessed fallback) plus the request/result types it is expressed in, with typed wire enums (`ImageSize`/`ImageQuality`/`OutputFormat`/`Background` — `JsonSchema`+`Deserialize` so the daemon's tool args reuse them directly). `OpenAiImageClient` is the sole adapter (OpenAI Images API, `gpt-image`-shaped): 180 s per-attempt wall-clock deadline (image renders are slow — far longer than a chat turn's budget), a deliberately **frugal 2-attempt retry budget** (a generation costs the provider real money, unlike a chat turn, so only clearly-transient 429/5xx get a second shot), and **no `response_format` field** — modern gpt-image models ignore it, and sending it makes older proxies 400, so the requested `output_format` alone carries the intent and the returned bytes are decoded with that MIME. The trait is a **sibling of `ProviderClient`, not a sub-trait**: generation is not a chat turn (different endpoint, different auth surface, no history), so the daemon resolves it through a separate handle rather than the chat client. Errors reuse `InferenceError`, so one error taxonomy and one metrics-label mapping serve both paths. Serialization is deliberately minimal — default knobs (`auto` size/quality, `png` format, `auto` background) are `skip_serializing_if`-omitted from the wire body, so an all-defaults request is just `{model, prompt, n}`: image models reached through OpenAI-compatible proxies (imagen, flux, gemini-image) often reject parameters they do not implement, even as explicit defaults. `Display` on the enums mirrors the serde wire strings exactly, so user-facing renderings (the tool's invocation line) show what the API receives. The adapter's wire tests live in `tests/images_wire.rs` (`#[ignore]`, socket-based — no `TcpListener` in unit tests). `from_account_config()` (daemon-side) is the future single dispatch point for additional backends (Responses-native images, Gemini imagen) — each becomes a new adapter behind the same trait, exactly like `ProviderClient`. |
