...
The ONE daemon-wide registry handle is created in `DaemonState::open`, handed
to every client constructor via `InferenceProvider::from_account_config`, and
to the request/cancel machinery via `RequestContext::socket_registry` — its
`Arc<Mutex<Vec<OwnedSock>>>` is a sanctioned shared-state exception (see
AGENTS.md): the lock is held only for a handful of fd syscalls, carries no
message traffic, and channels cannot express "un-block another thread's
blocked syscall", which is the whole point of the registry. `RegisteringTcpConnector`
...
shape as the config/ACL watchers); `handle_suspend_event` force-closes the
socket registry on `Sleep` (proactively — the logind event precedes
