...
`Undo`, `Redo`, `ContinueGeneration`, `GetImage`, `SetSessionPinned`, `SetSessionArchived`
...
`prune_dead` (a non-blocking `MSG_PEEK` probe, the same technique ureq's
`TcpTransport::is_open` uses) bounds growth; (2) `SocketTuning` — TCP
keepalive options applied once right after connect so a half-dead connection
(NAT timeout, cable pull) is noticed by the kernel. Registered sockets are
duplicated at registration (`try_clone` + `OwnedFd`); the registry closes
every fd it holds exactly once (explicit `into_raw_fd` + `close`, never
`Drop`, so an already-closed fd's EBADF is logged rather than double-closed).
Each SESSION owns a registry (in `SessionState`), plus one daemon-owned one
for non-session-scoped work (model prefetch, catalog fetch — never
individually cancelled); each provider client is constructed against the
registry of the unit of work that is going to use it
(`InferenceProvider::from_account_config(config, api_key, &registry)`).
The internal `Arc<Mutex<Vec<OwnedSock>>>` is cheap to clone, and the daemon
command loop holds one clone per live session (`DaemonState::session_registries`,
entered at session spawn, dropped at session exit) so it can force-close a
wedged session's sockets from the one thread that DECIDED the cancel — a
session thread blocked in a provider `read()` cannot observe a channel
message. Cancellation granularity is exactly the SESSION (a registry = a
session; sub-sessions own their own; a parent cancel closes the whole
subtree; future async tool calls will reuse the session's
agent+pool+registry triple — per-task registries were considered and
rejected). The lock is held only for a handful of fd syscalls, carries no
message traffic, and channels cannot express "un-block another thread's
blocked syscall", which is the whole point of the registry. RAII
deregistration completes the lifecycle: `register` returns a `SocketId`,
and `RegisteredTcpTransport` holds it plus a registry clone so its `Drop`
calls `unregister` — entry removal is the single close-ownership-transfer
signal, so if `shutdown_all`/`prune` already removed the entry the
unregister is a documented no-op and the guard can never double-close. In
steady state the registry therefore tracks only LIVE connections (ureq
closes pooled connections freely without leaking entries); the prune and
the 256-entry cap remain purely as backstops.
...
the platform API firing, so consumers must treat missed events as a non-issue
and rely on sockreg's kernel keepalives as the fallback. The daemon forwards
the events to its command loop as `DaemonCommand::PowerEvent` via a dedicated
`spawn_power_event_forwarder` thread (the same forwarder-into-command-channel
shape as the config/ACL watchers); `handle_suspend_event` force-closes EVERY
session's socket registry (plus the daemon's own) on `Sleep` (proactively —
the logind event precedes
suspension) and logs only on `Wake`.
...
| Module | Purpose |
|---|---|
| `openai/` | OpenAI-compatible client (`OpenAiClient`) covering both Chat Completions and Responses APIs (incl. programmatic tool calling), plus the canonical `ChatRequestMessage` / `ChatToolDefinition` types that the other clients translate into their own wire formats, `ServiceConfig`, SSE reader, and per-protocol retry helpers. Non-2xx HTTP bodies and Responses `response.failed` events both surface the provider's error `message` (the OpenAI error object `{code, message, param, type}` is decoded; a plain-string `error` or the serialized object is the fallback) so a mid-stream failure is never a blank error. For the opencode gateway providers (`provider_slug` exactly `"opencode"`, `"opencode-go"`, or `"opencode-go-anthropic-compatible"` — the shared allowlist lives in `shared::opencode_gateway_headers`, which both the OpenAI client and the Anthropic Messages path call), every turn request carries `x-opencode-session: <session id>`, `x-opencode-request: <request id>`, and `x-opencode-client: choreographr`, mirroring upstream's own client: the opencode.ai zen/go gateway filters to currently-healthy upstreams, then hashes the **last 4 characters** of the sticky id (the session header, else workspace id, else caller IP) to pick one and keys a sticky tracker on it, so real per-session ids spread sessions across upstream buckets and let the gateway's own health machinery route around broken providers. (An earlier revision sent a fixed `x-opencode-session: choreographr` to dodge a deterministic "Model is unavailable" upstream — that workaround pinned every choreographr session into a single hash bucket and was obsoleted once upstream gained server-side provider health management, so it was replaced with per-session identity.) Non-2xx handling aside, the Chat Completions adapter also parses the per-choice `finish_reason` (non-streaming and the final SSE chunk, threaded through the stream accumulator) into a lenient `FinishReason` enum that NEVER fails the parse — unknown values become `Other(raw)` with the raw string logged. Behavior mapping: `length` on a final-text turn sets `FinalTextResult::truncated` (the daemon appends a visible "⚠ response truncated (length limit)" notice; tool-call turns ignore it — `length` + tools is normal loop flow), whereas `length` with every tool call discarded as invalid/truncated JSON and no text becomes a `TruncatedToolCall` error (the daemon recovers and retries rather than rendering an empty "final" answer — see the agent loop below), `sensitive`/`content_filter` maps to the terminal non-retryable `ContentFiltered` error (same precedent as the images adapter), and `model_context_window_exceeded` maps to the distinct terminal `ContextWindowExceeded` error (a compaction-bug signal, metrics label `context_window_exceeded`); `stop`/`tool_calls`/`network_error`/unknown leave behavior unchanged. Every inference request across all protocols also carries `User-Agent: choreographr/<daemon version>` (the daemon injects its version into each client config's `user_agent` field; `build_agent` applies it in place of ureq's default), giving providers a named agent for metrics and allowlisting. Responses requests carry reasoning configuration in the nested `reasoning` object (`reasoning.effort`, plus `reasoning.summary: "auto"`), emitted only for reasoning-capable models (per `ServiceConfig::model_supports_reasoning`) — never as the Chat Completions top-level `reasoning_effort` and never via `include`; model-usage rejections (a not-a-chat-model or model-not-found response) are also rewritten into a short user-facing line (`'<model>' is not a chat model — …` / `… can't be used here — …`), with the provider's raw wording logged rather than shown. |
...
| `images/` | **Image generation clients** — the `ImageGenerationClient` trait (`generate_image(req, cancel_rx)`, `provider_slug()` — NO default-model surface: model choice is purely catalog/tool-side, never a guessed fallback) plus the request/result types it is expressed in, with typed wire enums (`ImageSize`/`ImageQuality`/`OutputFormat`/`Background` — `JsonSchema`+`Deserialize` so the daemon's tool args reuse them directly). Adapters: [`OpenAiImageClient`] (OpenAI Images API, `gpt-image`-shaped — inline `b64_json`) and [`ZaiImageClient`] (z.ai/Zhipu GLM Images API for the `zai` and `zhipuai` slugs — `glm-image` returns `data[0].url`, a temporary 30-day-expiring CDN link, so the adapter downloads the image itself: scheme-guarded http(s)-only, no Authorization header forwarded to the CDN (the URL is pre-signed; the key must not leak to a third-party host), `image/*`-loose content-type guard, and an in-stream 8 MiB download cap mirroring the daemon's display cap, plus an SSRF guard (`is_downloadable_url`, `url`-crate parse) rejecting IP-literal hosts in loopback/private/link-local/unique-local ranges — non-IP provider CDN hostnames are allowed (recorded residual-risk decision: provider-controlled hostname over an authenticated TLS channel, bytes fully validated downstream); request side sends `{model, prompt}` only plus `size` (verbatim WxH) and `quality` mapped to z.ai's `hd|standard` enum (`Low|Medium`→`standard`, `High`→`hd`, `Auto` omitted) and NEVER `n`/`output_format`/`background`/`seed` (undocumented fields are not gambling-tested — an explicitly-set `Background` is silently ignored per the best-effort-knob contract, documented in code); `content_filter` entries at level 0–2 (0 most severe per docs) map to a terminal `ContentFiltered` error with the flagged role — NOT `EmptyResponse`, and NO fabricated HTTP status (the response itself succeeded) — blocked ≠ retryable; the z.ai image base defaults to the documented standard PaaS base (`/api/paas/v4`) where images are actually served, so the coding-gateway rewrite (`/api/coding/paas/v4` → `/api/paas/v4`) is a no-op passthrough for the default chat base and only fires if an account's base_url override points at the coding gateway; and [`FalImageClient`] (fal.ai's SYNCHRONOUS image API for the `fal`/`fal-ai` slugs — `POST {base}/{model}` at `https://fal.run` returns the finished image in the SAME response, so there is no queue/polling; auth is `Authorization: Key <FAL_KEY>`, NOT the Bearer scheme; the request sends `prompt` plus `image_size` (the named enum `square_hd` where one matches our size exactly, else an explicit `{width,height}` object), `output_format` (`jpeg`/`png`, a requested webp falling back to png), and `seed` ONLY when the caller sets one (flux-2-pro documents an integer `seed` for reproducible output — the ONE place the shared `seed` knob is honored), and NEVER `quality`/`background` (not documented for flux-2-pro — silently ignored per the best-effort-knob contract) or `model` (the model IS the URL path segment); the response's `images[0].url` is normally a `fal.media` link the adapter downloads, or a `data:` URI under `sync_mode` which it decodes in place. fal's **TWO error shapes** (parsed by the shared `fal::error` mapper — see the `fal/` row) are switched on the machine-readable `type`/`error_type` (never the prose `msg`): the 422 `{"detail":[{loc,msg,type,…}]}` array maps `content_policy_violation`→`ContentFiltered` and `no_media_generated`→`EmptyResponse` (else a 422 client error), and the flat `{"detail":"…","error_type":"…"}` (the snake value also in the `X-Fal-Error-Type` header) maps `client_cancelled`/`client_disconnected`(499)→`Cancelled`, `request_timeout`/`startup_timeout`(504) + `runner_*`/`internal_error`→`ServerError`, and `bad_request`(400)→`ClientError`, with 401/429/other-5xx/other-4xx status fallbacks. Because the standard `retry_loop` consumes a terminal error body to summarize it, `FalImageClient` drives its POST through `retry::retry_loop_raw` — identical retry/backoff/cancellation, but a terminal non-2xx response is handed back UNCONSUMED so the two-shape mapper can read it (see the `retry.rs` row). The z.ai and fal download paths share the crate-level `download` module (hoisted out of `images/` so the video queue's result download reuses it — see the `download.rs` row): the scheme + SSRF guards (`is_http_url`/`is_downloadable_url`/`is_blocked_ip_literal`), `host_guard_relaxed` (relax the IP-literal host guard only for loopback/private bases), and the capped/retried/cancellable `download_media_bytes` (image callers go through the thin `download_image_bytes` wrapper) with its `NotReady`-vs-terminal distinction (a CDN propagation race retries; a scheme violation/cap overflow/empty body is terminal — never masked as `EmptyResponse`). All adapters share the 180 s per-attempt wall-clock deadline (also bounding the URL download, so glm-image's ~20 s `hd` render plus the CDN fetch stays in budget) and the **frugal 2-attempt retry budget** (a generation costs the provider real money, unlike a chat turn, so only clearly-transient 429/5xx get a second shot), extracted as the shared `super::IMAGE_*` constants, and **no `response_format` field** — modern gpt-image models ignore it, and sending it makes older proxies 400, so the requested `output_format` alone carries the intent and the returned bytes are decoded with that MIME. The trait is a **sibling of `ProviderClient`, not a sub-trait**: generation is not a chat turn (different endpoint, different auth surface, no history), so the daemon resolves it through a separate handle rather than the chat client. Errors reuse `InferenceError`, so one error taxonomy and one metrics-label mapping serve both paths. Serialization is deliberately minimal — default knobs (`auto` size/quality, `png` format, `auto` background) are `skip_serializing_if`-omitted from the wire body, so an all-defaults request is just `{model, prompt, n}`: image models reached through OpenAI-compatible proxies (imagen, flux, gemini-image) often reject parameters they do not implement, even as explicit defaults. The shared request's `seed: Option<u64>` is `skip_serializing` (not `skip_serializing_if`) on purpose: `OpenAiImageClient` composes its body by `#[serde(flatten)]`ing the request, so ANY serialized `seed` would leak to every OpenAI-protocol provider (gpt-image documents none) — suppressing it at the struct level forces each adapter to opt in explicitly, and only the fal adapter does (inserting it by hand from `req.seed`); a unit test pins that the OpenAI body never carries it. `Display` on the enums mirrors the serde wire strings exactly, so user-facing renderings (the tool's invocation line) show what the API receives. The adapter's wire tests live in `tests/it/images_wire.rs` (`#[ignore]`, socket-based — no `TcpListener` in unit tests). `from_account_config()` (daemon-side) is the single dispatch point for backend families (it routes the `zai`/`zhipuai` slugs to `ZaiImageClient` via `images::is_zhipu_image_provider_slug`, everything else OpenAI-protocol to `OpenAiImageClient`) — additional backends (Responses-native images, Gemini imagen, fal.ai) each become a new adapter behind the same trait, exactly like `ProviderClient`. A parallel helper `images::is_fal_image_provider_slug` recognises the `fal`/`fal-ai` slugs, but is consulted only where an IMAGE-ONLY backend is being resolved (fal has no chat API, so the daemon's chat-protocol dispatch cannot construct a `FalImageClient`), so the crate ships the adapter + slug helper for callers to construct directly. |
| `videos/` | **Video generation clients** — the `VideoGenerationClient` trait, whose primitive is a **queue handle** (`submit(&req) -> VideoJobHandle`, `poll(&handle, want_logs) -> VideoJobStatus`, `result(&handle) -> VideoGenerationResult`, `cancel(&handle)`, `provider_slug()`) plus a **provided blocking driver** `generate_video(req, cancel_rx, on_progress)` that chains submit→poll→result, emits progress, honours `cancel_rx` between polls (event-driven via `retry::sleep_or_cancel`, cancel arm biased first) and on cancel/deadline best-effort calls `cancel()` before returning `Cancelled`/`DeadlineExceeded`. The handle (not a single blocking call) is the primitive so a caller can submit several clips in parallel and poll them together — the "render the next shot while this one plays" pattern. Types: `VideoJobHandle { request_id, provider_slug, model, status_url, response_url, cancel_url }` (fal's returned URLs used verbatim, not recomposed), `VideoJobStatus { Queued{position}, InProgress{logs}, Completed{metrics}, Failed{detail, error_type} }`, `VideoGenerationRequest { prompt, model, duration_secs?, resolution?, aspect_ratio?, seed?, negative_prompt?, from_frame_url?, end_frame_url? }`, `VideoGenerationResult { url, content_type?, file_size?, seed?, expanded_prompt?, model }` — a **URL**, never inlined bytes (video is tens of MB; the H3 `sync_mode` base64 path is deliberately avoided). Wire enums `VideoResolution`/`VideoAspectRatio` (`JsonSchema`+`Serialize` like the image enums); request knobs are `skip_serializing_if`-default-omitted. Policy consts: `VIDEO_TOTAL_TIMEOUT_SECS` (900 — fal exposes no caller-settable total-inference deadline for queue jobs, so the driver owns the job budget), `VIDEO_POLL_INTERVAL_MS` (2000), `VIDEO_DOWNLOAD_CAP_BYTES` (256 MiB). `is_fal_video_provider_slug` (`fal`/`fal-ai`). **Adapter [`FalVideoClient`]** targets fal's video **queue** (`https://queue.fal.run`, `Authorization: Key <FAL_KEY>`): `submit` POSTs `{base}/{model}` and is deliberately **NOT retried** (fal re-queues failed runners up to 10× server-side, so a duplicate submit would double-bill), while `poll`/`result`/`cancel` are idempotent GET/PUT and DO use the retry layer; `cancel` maps fal's `202 CANCELLATION_REQUESTED`/`400 ALREADY_COMPLETED`/`404 NOT_FOUND` to `Ok` (nothing to cancel is not a failure). A per-model body mapper (`build_fal_video_body`) covers `minimax/h3-max/{text,image}-to-video` exactly (required `prompt_expansion_mode` pinned `balanced`; `duration` 5–15 int; **uppercase** `480P`/`768P`/`1080P`; optional `image_url`/`end_image_url`) and generic families (veo3/kling/wan/seedance) via a prefix-detected mapping (lowercase `p`, per-family duration form); an unknown model gets the generic mapping. Results are `{"video":{url,content_type,file_name,file_size}}` (a single object; **no** width/height/duration in the schema). fal's two error shapes are parsed by the shared `fal/` mapper, and a **third** site — a `COMPLETED` status body carrying `error`/`error_type` for a failed job — routes through the same classifier (unknown ⇒ `ServerError`). Constructed directly by callers (fal has no chat API, so the daemon's chat-protocol dispatch cannot build it — see the `images/` row); the daemon media-only-provider path is a follow-up. `Director` is a WebRTC realtime stream, deliberately NOT modelled here. Wire tests live in `tests/it/videos_wire.rs` (`#[ignore]`, keyless, socket-based). |
| `fal/` | Shared fal.ai wire-contract helpers used by BOTH fal adapters (hoisted out of `images/fal.rs` so `FalImageClient` and `FalVideoClient` cannot drift). `error.rs` owns the two-shape error mapper: `fal_error(status, x_fal_error_type, retry_after_secs, body) -> ProviderError`, with `parse_model_validation_error` for the 422 `{"detail":[{loc,msg,type,url,ctx?,input?}]}` array, `parse_request_error` for the flat `{"detail":"…","error_type":"…"}` shape, and `status_fallback_detail` for the 401/429/5xx/4xx HTTP fallbacks. Switched on the machine-readable `type`/`error_type` (never the prose `msg`): `content_policy_violation`→`ContentFiltered`, `no_media_generated`→`EmptyResponse`, `client_cancelled`/`client_disconnected`→`Cancelled`, `request_timeout`/`startup_timeout`/`runner_*`/`internal_error`→`ServerError`, `bad_request`→`ClientError`. `fal_error_from_response(response, context)` is the single entry point both adapters use for a terminal non-2xx response (it reads the `X-Fal-Error-Type` header, the `Retry-After`, and the body, logs, and applies `fal_error`), so their error handling cannot drift. |
| `download.rs` | Crate-level shared URL-download machinery (hoisted out of `images/` so the video queue's result download reuses it): the scheme + SSRF guards (`is_http_url`/`is_downloadable_url`/`is_blocked_ip_literal`), `host_guard_relaxed(base_url)` (relax the IP-literal host guard only for loopback/private bases — mock providers and local proxies — while the scheme half always applies), and the capped/retried/cancellable `download_media_bytes(http, config, url, content_type_prefix, cap, cancel_rx)` with its `NotReady`-vs-terminal distinction (a CDN propagation race retries; a scheme violation/cap overflow/empty body is terminal — never masked as `EmptyResponse`). The per-call `content_type_prefix` (`image/` vs `video/`) and `cap` let the image path (8 MiB) and the video path (`VIDEO_DOWNLOAD_CAP_BYTES`, 256 MiB) share one implementation with the cap enforced during the streamed read; image callers go through the thin `download_image_bytes` wrapper. The download GET disables redirect **following** (`max_redirects(0)`) and rejects a non-2xx with a retryable `NotReady` before reading any body, so a provider-controlled 3xx cannot steer the fetch past the SSRF guard to an address the guard never inspected. |
...
catalog. All other cross-thread communication in this crate uses crossbeam
channels, as do the daemon's long-lived command channels (`DaemonCommand`,
`SessionCommand`).
...
| Module | Purpose |
|---|---|---|
| `broadcast.rs` | The ONE shared subscriber-broadcast policy: **lossless delivery with lag-based eviction**. Each subscriber is a [`SubscriberSink`](choreo-daemon/src/broadcast.rs) — an UNBOUNDED crossbeam channel plus an `Arc<AtomicUsize>` in-flight byte counter (the 6th sanctioned shared-state exception, see AGENTS.md) — so the daemon never drops a broadcast and a slow subscriber can never stall a session thread or the command loop. Every enqueue (`enqueue`, `send_unchecked`, and the connection thread's `send_to_writer` via the shared `send_accounted` core) increments the per-client and daemon-wide (`global_lag`) counters before the send and self-corrects both when the send fails on a dead receiver; the connection's writer thread decrements both on every dequeue and drains-and-decrements whatever is still queued when it stops early, so the accounting stays balanced to within one bounded exit-window straggler (see the module docs). Crossing [`LagLimits`](choreo-daemon/src/broadcast.rs) (`per_client_cap` 64 MiB, `global_budget` 512 MiB, injectable for tests) reports `ClientOverLag`/`GlobalOverBudget` and the fan-out caller evicts the client(s) via `DaemonCommand::EvictClient`/`EvictLargestLagging`. The retain-and-evict collection itself is the single `fan_out_evicting` helper, shared by ALL THREE subscriber fan-outs (daemon summary, daemon all-activity, per-session) so the policy cannot drift between paths. `SessionStatusChanged` is the one message that would otherwise flow through all three for a single event — the session thread broadcasts it per-session AND forwards it to all-activity AND sends the summary command — so the summary fan-out (in `daemon/subscriber_handlers.rs::handle_broadcast_session_status`) skips clients that already received it via the per-session or activity fan-outs, making delivery exactly-once per client class (pinned by `handle_broadcast_session_status_dedups_against_session_and_activity_subscribers`). Daemon-generated LIFECYCLE events with NO session-thread path — `SessionCreated`, `SessionDeleted`, and the exit `Sleeping` status — are the one class that cannot dedup against a `BroadcastActivity` forward, so `DaemonState::broadcast` (in `daemon/subscriber_handlers.rs`) delivers them to BOTH the session-summary subscribers AND the all-activity subscribers: the summary fan-out skips all-activity clients that the activity fan-out already served, so an activity-only client now learns of sessions being created/deleted while a dual subscriber still gets exactly one copy (pinned by `broadcast_lifecycle_delivers_to_summary_and_activity_exactly_once_per_client`). The all-activity fan-out has its own duplicate-suppression in `DaemonCommand::BroadcastActivity { session_id: Option<u64>, msg }` / `handle_broadcast_activity`, keyed on **explicit provenance, not message shape**: the origin session id travels on the command — `Some` for session-originated broadcasts (the sending session thread knows its own id), `None` for global/control broadcasts — and `handle_broadcast_activity` passes a `should_skip` closure to `fan_out_evicting` that skips clients which are also direct subscribers of that origin session (they receive the event through the per-session path instead). Every session-scoped event still rides the `DaemonMessage::Session { session_id: Option<u64>, event }` envelope, so the origin is present on the wire too, but the filter reads ONLY the command field — a `Some(42)` origin suppresses delivery of any message, whatever its shape, to a session-42 subscriber (pinned by `handle_broadcast_activity_dedup_keyed_on_command_origin_not_message_shape`). Because the command provenance and the message origin must AGREE — a `Some` origin on a NON-session message would drop it for the origin session's direct subscribers on BOTH paths (deduped on the activity path and never carried on the per-session path), and a `Session` envelope whose own `session_id` differs from (or contradicts) the command's would dedup against the wrong session — `handle_broadcast_activity` guards the contract with a `warn!` tripwire: `violates_broadcast_origin_contract` in `subscriber_handlers.rs` (also flags a `None` command origin on a session-scoped envelope, which would double-deliver to the envelope origin's direct subscribers), pinned by `broadcast_origin_contract_requires_agreeing_provenance` — so a future producer that forwards a non-session message with `Some`, or an envelope whose origin disagrees with the command, fails loudly instead of silently mis-routing messages (no current producer does this). `approx_wire_size` (choreo-proto, `size.rs`) is the gauge — deliberately an over-estimate, pinned by `approx_wire_size_never_underestimates_encoded_payload`. `counts_toward_client_lag` (keyed on the message variant, applied identically by `send_accounted` and by the writer's dequeue/drain) EXCLUDES a solicited `Image` reply from the PER-CLIENT counter — a `GetImage` answer is request-driven and can be multi-megabyte (up to `MAX_FRAME_SIZE`, equal to the default per-client cap), so counting it could evict a client merely for scrolling images into view; it still counts in the daemon-wide budget, and a broadcast still counts toward the per-client cap (pinned by `image_reply_is_excluded_from_per_client_lag_but_a_broadcast_is_not`). |
| `server/lifecycle.rs` | Transport ADAPTERS over `&DaemonCore` (assembled in `server/core.rs`): accept loop (blocking `UnixListener` + signal-wakeup), signal handling, metrics server spawn, and shutdown orchestration. On graceful shutdown, `DaemonMessage::ShuttingDown` is routed through each connection's single writer thread (via a `client_writers` registry in the daemon command loop); the writer thread flushes it and closes its own socket, so a client observes the notification before the EOF. The accept-loop thread never writes to or closes client sockets — there are no retained stream clones and no backstop close pass, so the notification cannot be lost to a race with a socket close. With the lossless unbounded writer channels an enqueue can never be `Full` (the old bounded round-robin fan-out for full channels is dead code); a wedged writer is bounded by its socket write timeout (`DaemonState::writer_write_timeout`, default 5 s) plus the writer-join grace. The writer channel is created and REGISTERED with the daemon before the connection thread is spawned (see the `server/connection.rs` row's `register_client_writer`), so a connection accepted concurrently with shutdown is guaranteed to be in the registry when the broadcast is processed — the register is ordered before the broadcast on the FIFO command channel (Unix: same thread; TCP: the accept thread is joined before the broadcast). Before returning, `run_server` also waits — bounded, 5 s — for the TCP accept thread (woken by a probe connect to the listener's actual bound address, falling back to loopback for unspecified `0.0.0.0`/`::` binds, so a concrete non-loopback bind is woken too; the probe's own connect is bounded by a 1 s `connect_timeout` so a full accept backlog cannot stall shutdown on the kernel's SYN-retry timeout) and then for each connection thread (Unix handles tracked directly, TCP handles ferried back over a channel) and, through it, its writer thread, to flush the notification and close its own socket, so notify-before-EOF holds even when `run_server` is embedded in-process rather than exiting the process. Before binding, the accept path runs `remove_stale_socket`: an existing socket file is probed with a connect, and a SUCCESSFUL connect means a live daemon is still listening — startup fails with "another daemon is already listening" instead of unlinking the working daemon's socket (the two-daemon race the TUI autostart would otherwise widen); a failed connect (ENOENT, ECONNREFUSED, a regular file) means stale, and the leftover is removed with the path carried in the error. With `--auto-exit`, each connection thread (both transports) sends `DaemonCommand::LastClientDisconnected` on its exit — AFTER dropping its `ConnectionSlot`, so the command loop's count is the true live total — and the command loop (see the `server/core.rs` row) drives the shutdown from there. Connection handles are pruned eagerly once the retained Vec grows past 64, so a long-running daemon does not accumulate one `JoinHandle` per connection ever accepted (pinned by a unit test). Live connections are also capped at 256 (`MAX_CONCURRENT_CONNECTIONS`, both transports combined, pinned by unit tests + an integration test): at the cap a newly-accepted connection is dropped immediately (bare EOF) so wedged-but-open clients cannot exhaust thread/FD resources. `cleanup_client`'s writer join is bounded (5 s `WRITER_JOIN_GRACE`) so a writer wedged in a blocking socket write cannot hang its connection thread's cleanup; a timed-out writer is detached and the shutdown drain remains the backstop. The cap is enforced with a daemon-wide `Arc<AtomicUsize>` live-connection counter (RAII `ConnectionSlot`) shared across both accept paths and every connection thread's exit — a single-purpose, lock-free resource-accounting exception to the workspace's message-passing rule (documented in AGENTS.md). Signal handling is channel-driven on both platforms: Unix blocks in `signal_hook`'s iterator (self-pipe); Windows has no sigwait/iterator, so `low_level::register` forwards SIGINT/SIGTERM as channel messages and the handler thread blocks in `recv()` — both wake the accept loop via a connect to the daemon's own socket, with no flag polling. On both platforms the handler is REGISTERED SYNCHRONOUSLY on the `run_server` thread *before* `remove_stale_socket`/the socket bind, and only then is the registered iterator (Unix) or channel receiver (Windows) moved into the consumer thread: `Signals::new`/`low_level::register` install the handler as a side effect of returning, so the ordering guarantees that once the socket file exists (or the TCP listener is connectable) the handler is already live. The previous shape registered *inside* the spawned thread, so a SIGINT delivered between the bind and that thread's first instruction hit an uninstalled handler and terminated the process by the kernel's default action — the startup race pinned by `sigint_exits_fresh_daemon_with_no_connections` in `ctrlc_after_connection`. |
| `server/core.rs` | Transport-INDEPENDENT daemon assembly: `start_daemon_core(state, CoreOptions)` creates the command channel, installs the shared ACL (`CoreOptions::acl: Option<_>` — `None` is the future embedded-daemon case, which owns its ACL policy upstream; skipped wholesale so a half-installed ACL can never diverge), spawns the ACL + config watchers (`CoreOptions::config_watchers: bool`; `false` degrades to a `crossbeam_channel::never()` overlay receiver exactly like the no-config-dir fallback), spawns the catalog-maintenance thread, creates the shutdown flag and the daemon-wide live-connection counter (`DaemonCore::conn_count` — created here so the shared Arc exists before either transport adapter is spawned), spawns the `choreo-power-events` monitor (`PowerMonitor::best_effort` — inert fallback where no platform mechanism exists) plus its `spawn_power_event_forwarder` thread (blocks in the monitor's receiver — a dedicated thread blocking in `recv` is fine, the house rule only forbids blocking the command loop — and sends each event as `DaemonCommand::PowerEvent`, the same forwarder pattern as the config/ACL watchers keeps every external event source on one uniform delivery shape rather than special-casing the loop with an extra `select!` arm), clones `global_lag` out of `DaemonState` before `state` moves into the command-loop thread (and likewise the shared `Arc<redb::Database>` into `DaemonCore::db`, so the transport adapters can hand each connection thread its own handle), spawns that thread, and returns the `DaemonCore` bundle (`daemon_tx`, `shutdown`, `global_lag`, `conn_count`, `db`, `cmd_handle`). The command loop owns the `--auto-exit` decision: `CoreOptions::auto_exit_wake_path: Option<String>` (set by the lifecycle adapter to the daemon's own socket path when the flag is passed; `None` for the embedded daemon — auto-exit needs a socket to wake) and a private clone of the connection counter. On `DaemonCommand::LastClientDisconnected` with a zero count (any other count is a no-op — a client left while others remain) it sets the SAME shutdown flag the accept loop polls and self-connects to the wake path, reusing the exact SIGINT shutdown path (drain, notify-before-EOF, unlink) instead of a parallel teardown. There is deliberately no idle timer: a daemon that has never had a client runs forever. No listener is touched here, so the Unix daemon and the future embedded daemon share this exact assembly; the shipped binary's behavior is unchanged (the moved code is verbatim from the pre-split `run_server`). |
| `server/connection.rs` | Per-client `client_thread` (Unix), `tcp_client_thread` (TCP/Noise), and `embedded_client_thread` (in-process, spawned by `crate::embedded`) — read `ClientMessages` from the transport (socket bytes, or channel values on the embedded path), dispatch via `daemon_tx` crossbeam channel. Single-writer discipline: each connection has exactly one writer thread draining its `writer_rx`; `ShuttingDown` AND `Evicted` are special-cased messages that make the writer flush, close the socket (or, on the embedded path, DROP the channel sender — the channel analogue of `Shutdown::Both`), and stop draining (notify-before-EOF / advisory-before-EOF), and no other thread ever writes to the transport. The writer loops of all three transports share one implementation (a `ConnectionWriter` trait + `writer_thread`; the embedded `ChannelConnectionWriter` forwards `DaemonMessage` VALUES — a value clone per message replaces the socket path's msgpack encode + AES-GCM encrypt + syscall), so the sole-writer contract and the special-case closes live in exactly one place — pinned by unit tests with a mock `ConnectionWriter` (flush `ShuttingDown`, flush `Evicted`, stop on send error, end cleanly on disconnect) and channel-writer tests (send-after-shutdown errors; receiver sees the close). The embedded connection thread uses the same `ClientConn` (`is_unix: true` — an in-process connection is the LOCAL trust domain, so `/acl add` applies) with NO error classification and NO timeouts: `for msg in client_rx` ends exactly when the client drops its link (channel close IS the EOF). The writer channel is an UNBOUNDED crossbeam channel (the connection's `SubscriberSink`), created and registered with the daemon by `register_client_writer`, called by the acceptor BEFORE the connection thread spawns (see the `server/lifecycle.rs` row) — a failed TCP handshake unregisters via `ClientDisconnected` so the registry stays honest. The writer thread decrements the sink's in-flight byte counter (and the daemon-wide `global_lag`) on every dequeue, the exact counterpart of the producers' enqueue increment; on a send error (broken pipe, or the 5 s `WRITER_WRITE_TIMEOUT` on a wedged client whose receive window is zero) it shuts the socket down itself — unblocking the reader's blocking read so `cleanup_client` reaps the connection. Replies are written through `send_to_writer`, an unbounded `send_accounted` call that still increments (and self-corrects on a dead receiver) the byte counters EXCEPT that a solicited `Image` reply is excluded from the PER-CLIENT counter (`broadcast::counts_toward_client_lag`, mirrored by the writer's dequeue/drain) so a client is never evicted for fetching its own images — a `GetImage` answer can be multi-megabyte, up to `MAX_FRAME_SIZE`; it still counts in the daemon-wide budget (replies were never dropped — a blocking send just blocked; with unbounded channels they can no longer block either). Session-summary subscription is an explicit client decision on both transports: a client opts into `SessionCreated`/`SessionStatusChanged`/`SessionDeleted` push broadcasts with `SubscribeSessionsSummary` (previously `tcp_client_thread` auto-registered every Noise client on connect; the GUI now sends the subscribe message at connect to keep its session list live). `cleanup_client` joins the connection's writer thread with a 5 s bound (`WRITER_JOIN_GRACE`) so a writer wedged in a blocking socket write cannot hang the connection thread's cleanup — a timed-out writer is detached and exits on its own once the client goes away. Each connection thread ALSO carries its own `Arc<redb::Database>` (cloned from `DaemonCore::db`, threaded through every transport: `client_thread`/`tcp_client_thread`/`tcp_handshake_and_client_thread` and `EmbeddedConnArgs`): `handle_client_get_image` reads the requested `session_attachments` slot DIRECTLY on the connection thread via `db::read_display_image` — no command-loop round-trip, no reply channel — using the same trust boundary (`*ctx.attached_session_id == Some(session_id)`, else `None`) and mapping a redb error to `None` with a `warn!` exactly as the removed command-loop handler did. redb's `Database` is designed for concurrent readers, so a single shared handle across every connection thread is safe; the handle lives on the connection's `ClientConn` for its whole lifetime (see `ClientConn::db` / `ClientCtx::db`). |
| `embedded.rs` | The embedded (in-process) daemon transport — the third transport, next to the Unix socket and TCP/Noise. `spawn_embedded(state, EmbeddedOptions)` assembles ONLY the transport-independent core via `start_daemon_core` (no Unix socket, no TCP listener, no signal threads, no metrics); `EmbeddedDaemon::connect()` mirrors the TCP accept arm — `ConnectionSlot` (MAX_CONCURRENT_CONNECTIONS applies uniformly across all three transports), `register_client_writer` BEFORE spawning (the shutdown ordering invariant: a concurrently-shutting-down client still receives `ShuttingDown`), then `embedded_client_thread` over plain channels, with the JoinHandle ferried to `shutdown()` over a channel (same pattern as the TCP accept thread). The connection NEVER becomes bytes: `ClientMessage`s travel client→daemon as values on one channel, `DaemonMessage`s daemon→client as values via `ChannelConnectionWriter` (see the `server/connection.rs` row); channel close IS the EOF in both directions and there is NO polling and NO serialization anywhere. `connect()` returns only after the connection thread is spawned, so early client sends simply queue (no "socket not ready" window — there is no handshake). `shutdown(self)` runs the same drain `run_server` does — `BroadcastShuttingDown` then `Shutdown` on the FIFO command channel, join the command loop, drain the handle ferry and bounded-join every connection thread against the shared `CONNECTION_DRAIN_GRACE` — so the client observes `ShuttingDown` as a VALUE, then the channel close (pinned by `shutdown_delivers_shutting_down_before_channel_close`). Dropping an `EmbeddedDaemon` without `shutdown()` is a documented defect surfaced by a loud `info!` (the command loop closes on the dropped `daemon_tx`, but no broadcast is sent and no connection threads are joined); `shutdown()` is the required path. |
...
| `daemon.rs` | `DaemonCommand` handler loop on a dedicated thread — session CRUD, attach/detach, listing, locking, account management (incl. `AccountsReload`, the external-edit watcher consumer: re-reads `accounts.toml`, parse-compares against the in-memory manager, applies a real change, invalidates affected sessions' cached clients via `SessionCommand::DropProvider` for removed/modified accounts and pushes their refreshed provider slug (`SessionCommand::SetProviderSlug`, `None` on removal) so slug-keyed catalog facts stay exact, and broadcasts the fresh list) — both fan-outs share the `for_each_session_bound_to` helper so they target exactly the same session set; the per-account→slug mapping is the single `account_provider_slug` helper, the runtime catalog swap (`CatalogBaseChanged` → `replace_catalog`, the single writer of the `PROVIDER_CATALOG` ArcSwap), `CatalogUpdated` broadcasts, and `/refresh-models` plumbing (the fetch is delegated to the maintenance thread, never run here). **Per-session registries live here too**: `DaemonState::session_registries` maps every live session to a CLONE of its `SessionState::registry` (entered in `spawn_session` before the session thread spawns, dropped in `handle_session_exited`) so the command loop can force-close a wedged session's sockets from the thread that decides the cancel — the ONLY shared handle left from the old daemon-wide registry design; everything else is per-session. **Cancellation force-closes the TARGET SESSION's registry too**: `handle_cancel_request` (the site where a cancel is DECIDED — the session worker only observes it) calls `force_close_session_sockets(session_id)` — a `shutdown_all` on that session's registry clone (and, via `cancel_children_of`, on every child's) — with an info log distinguishing the user-cancel reason from suspend and from organic provider IO errors (which never trigger a shutdown); unrelated sessions' connections are untouched (pinned by the cancel-isolation unit tests). Model lists are likewise warmed off the command loop: a detached prefetch thread runs on session join/attach/account-switch (`maybe_spawn_model_prefetch`, gated by the shared `MODEL_CACHE_TTL` freshness + a per-account in-flight dedup guard) and reports back via the `ModelPrefetchResult` command, which is the loop's cache insert — prefetch builds its throwaway client against the daemon-owned `daemon_registry` (never individually cancelled), with no network I/O on the command loop itself. **Keystore binding commands live in the child module `daemon/keystore.rs`** (see that row); the cached `keystore_bound` flag itself (read from the DB at `DaemonState::open`) backs the three-state keystore status: `current_keystore_message`/`broadcast_keystore_state` push `DaemonMessage::Keystore { state }` (`Unbound` when unbound, else `Locked`/`Unlocked`) to each activity subscriber at subscribe time and on every transition. These `DaemonCommand`s carry the acting client's `SubscriberSink` so the command loop can enqueue the targeted reply (`Unlocked`/`Bound`/`KeystoreUnbound`/`LockedError`/`CredentialAdded`) via `send_targeted` BEFORE any lock-state transition broadcast — the ORDERING INVARIANT the client's key-recording correctness keys on (pinned by `unix_targeted_reply_precedes_lock_state_broadcast`). `DaemonState` is owned by this thread only (no shared state); it is CONSTRUCTED by `daemon/open.rs` (see that row). **Image-generation resolution lives in the child module `daemon/image_provider.rs`** (see that row). The per-client subscriber/eviction methods — registration, the lossless broadcast fan-outs, lag eviction, shutdown notification, and disconnect cleanup — live in the child module `daemon/subscriber_handlers.rs` (`pub(super)` methods on `DaemonState`; `handle_command` dispatches their `DaemonCommand` variants here) so this file stays focused on core command handling. **Power events land here too**: `DaemonCommand::PowerEvent(SuspendEvent)` — forwarded from the `choreo-power-events` monitor by a dedicated thread in `server/core.rs` — dispatches to `handle_suspend_event`: `Sleep` force-closes the `daemon_registry` AND every session's registry (`shutdown_all` each; `machine sleeping: force-closing N provider sockets` log — the event precedes suspension, so this is the one proactive window), `Wake` prune-probes every session + daemon registry (a missed `Sleep` leaves dead entries registered; normally a no-op since the Sleep arm already emptied them) and logs — keepalive tuning plus the prune cover stragglers — the closure/prune is deliberately NOT a correctness dependency, see the choreo-power-events section). |
| `daemon/keystore.rs` | The keystore command handlers, lifted out of `daemon.rs` (same child-module pattern as `daemon/subscriber_handlers.rs`): the `Unlock`/`BindKeystore`/`Lock`/`AddCredential` handlers enforce the verify-only vs bind split (`handle_unlock_inner` / `handle_bind_keystore_inner` — the latter is the ONLY adoption path, TOFU on an unbound keystore) and run the shared `unlock_tail` (bulk-decrypt, load accounts, clear `locked`) — split for atomicity: `decrypt_credential_blobs` does the fallible I/O, and `finish_unlock` commits `credentials`/`accounts`/`locked = false` ONLY after every fallible step on locals succeeded, so a failing unlock never publishes decrypted plaintext into reachable state (the earlier pre-account-load order did — fixed); the `AddCredential` implicit unlock uses `unlock_tail_skip`, which seeds the just-test-decrypted blob instead of re-decrypting it from the DB. The cached `keystore_bound` flag is flipped `true` here (in `bind_keystore`) — the single-writer adoption path — backing the three-state keystore status: `current_keystore_message`/`broadcast_keystore_state` push `DaemonMessage::Keystore { state }` (`Unbound` when unbound, else `Locked`/`Unlocked`) to each activity subscriber at subscribe time and on every transition. These `DaemonCommand`s carry the acting client's `SubscriberSink` so the command loop can enqueue the targeted reply (`Unlocked`/`Bound`/`KeystoreUnbound`/`LockedError`/`CredentialAdded`) via `send_targeted` BEFORE any lock-state transition broadcast — the ORDERING INVARIANT the client's key-recording correctness keys on (pinned by `unix_targeted_reply_precedes_lock_state_broadcast`); a `/lock` against an UNBOUND daemon broadcasts no transition (already-`Unbound` for the subscribers, so the broadcast is redundant chatter). The pure keystore SEMANTICS (binding adoption/verification, crypto, blob storage) stay in the `choreo-keystore` crate — this module is the daemon's ORCHESTRATION of them. |
...
| `providers/mod.rs` | `InferenceProvider` — protocol-erased facade wrapping `Arc<dyn ProviderClient>` plus the catalog slug. `from_account_config()` takes the `SocketRegistry` of the unit of work that will use the client — a SESSION's registry for session-scoped requests (session-owned lazy clients; sockets register into that session's registry, so a cancel/suspend `shutdown_all` is scoped to exactly that session), or the command-loop-owned `daemon_registry` for non-session-scoped work (model prefetch, catalog maintenance) — and dispatches by `ProviderProtocol`, constructing the right client from `choreo-ai-protocols`. OpenAI-protocol accounts additionally get an `OpenAiImageClient` built from the SAME `ServiceConfig` clone (same key/base_url/user agent/slug) stored as `image_client: Option<Arc<dyn ImageGenerationClient>>`; Anthropic/Gemini leave it `None` (image backends deferred in v1). The accessor `image_client()` clones the Arc, and `ImageProviderHandle { slug, client }` is the opaque payload `DaemonCommand::GetImageGenerationProvider` hands to tool threads — slug included because the tool does catalog-based model selection and the client's own `provider_slug()` may be generic. Records API metrics (`record_api_call`/`record_api_error`) around each turn — timing lives here, not in the provider crate. |
| `sessions.rs` | `SessionState` (split into `SessionConfig` for persisted fields + runtime state) — now also owning this session's `SocketRegistry` and a LAZILY-built provider client (`provider: Option<InferenceProvider>`; sessions can exist while the keystore is locked, so the client is built on the session thread at the first request against the session's registry, and is dropped-on-`/lock` via `SessionCommand::DropProvider` to rebuild from fresh credentials), plus the account's **provider slug** (`provider_slug`, recorded as a NON-SECRET fact the moment the account config is known — at spawn time from the command loop's `AccountManager`, in lazy provider resolution, and on `SetAccount` even before unlock), `RequestContext`, and the `SessionCommand` enum with its handlers. The slug exists because a model's static catalog facts must never wait for the credential-bound client: the reasoning capability and the fallback context-window lookup are resolved from the recorded slug (`model_reasoning_capability`, `lookup_context_window` — both are pure catalog reads), while a live client still gets first say on the context window (its per-model client-config override) — so static facts are exact on a locked keystore / pre-first-request session instead of blinking out until the next request (the reported regression: glm-5.3-flash's 1M window and Alt+R's effort levels). `SetAccount` drops the PREVIOUS account's cached client on a real account switch (a stale client would otherwise keep serving the new account name — `resolve_provider` returns any cached client unconditionally), records the new account's slug even when no client can be built, and re-resolves the context window from the slug; `SessionCommand::SetProviderSlug` lets the daemon's accounts-reload path refresh the recorded slug (or clear it, on removal) without waiting for the next request. Each session has a control thread running `session_main()`; request work runs on separate worker threads via `run_request_worker()`. Sessions form a tree (parent → child sub-sessions), each with an optional working directory. |
...
| `tools/context.rs` | `ToolContext` — session-scoped context (session ID, `Arc<Database>`, `crossbeam_channel::Sender<DaemonCommand>`, active tool groups, reasoning effort, selected model, working directory, and the session's discovered-skill snapshot) passed to tools that need DB or daemon access or parent config for sub-sessions. |
...
| `tools/image_gen.rs` | The `GenerateImage` tool (group `"image"`, registered alongside the other optional groups) — text-prompt → image bytes via the session's image-capable provider. Deliberately thin: provider resolution lives in the daemon command loop (`GetImageGenerationProvider`, so the credential never reaches a tool thread beyond the TEMPORARY slot), model selection is a small pure catalog-driven function — explicit `model` arg wins (a bad name surfaces the provider's clean 4xx, the right failure for a deliberate pass-through), otherwise a priority pick over ONLY catalog-verified candidates (`image_models_for_provider`: gpt-image > imagen > gemini-image > flux > dall-e > first listed — never guessed, an un-catalogued model is unlikely to be routable), and when the catalog lists NO image models at all the tool fails with guidance (pass `model` explicitly, or add a `supports_image_output` overlay entry + /refresh-models) — there is NO guessed fallback model, because a guessed model the provider does not route (e.g. `gpt-image-1` on an opencode gateway) silently sends a doomed request; fail-safe over fail-hopeful — and the returned base64 re-enters the EXACT `prepare_image_from_bytes` pipeline `display_image` uses (extracted from `tools/image.rs` for this, behavior-neutral) plus the same `DisplayImageReturn`, so the generate→display→persist→vision-feedback loop (the model SEES what it generated and can refine on the next turn) and durable persistence are free with zero proto/client changes. The tool checks `ToolContext.cancelled` before send AND re-checks it the moment the round-trip returns, so a cancel issued while a (up to 180 s) generation was in flight discards the result before decode/validate/persist/display (the sanctioned cooperative-flag exception), and its 180 s per-attempt deadline + frugal 2-attempt retry live in the adapter (`images/` row in choreo-ai-protocols). Covered by the `#[ignore]` integration test `choreo-daemon/tests/it/image_gen_integration.rs` (full pipeline via `UnixStream::pair()`-style handler plumbing). |
...
| `state/command_palette.rs` | The inline command palette (Chat page). There is **no `command_mode` flag**: a line is a COMMAND LINE iff the shared composer buffer (`App::input`) starts with `/`, and the palette is shown exactly when it is (`command_palette_active` = buffer starts with `/` && model selector closed — TRUE even with zero matches so a "no matches" palette still shows and `Enter` still submits the line). The `/` therefore lives IN the buffer, verbatim as typed and exactly as `parse_input_line` expects (it strips one leading `/`), so typing the leading `/` shows the palette and deleting it hides it — no latched flag to desync from the buffer. `command_query` strips the `/` and takes the first whitespace token to filter `match_commands`; `command_palette_enter_line` resolves the line `Enter` runs (a first token that already names a command EXACTLY runs verbatim with its arguments preserved, while an empty or still-partial token adopts the HIGHLIGHTED row keeping any argument tail, so `Enter` alone runs the selected command with no preceding `Tab`; with no highlighted match the line runs verbatim so the parser still reports an unknown command); `command_palette_complete` rewrites the buffer to `/<name>` keeping any argument tail the user already typed (`/mo gpt-4o` → `/model gpt-4o`, agreeing with `command_palette_enter_line`) — or `/<name> ` with a trailing space when there is no tail — all without submitting. Owns the palette's transient selection state (`CommandPaletteState` — highlight index + scroll hint) and its wrap-around `↑`/`↓` movement, and `discard_command_line` (a guarded no-op on a non-command buffer, so a real prompt draft survives; invoked from `App::set_page` / `reset_for_session_switch` / `attach_to_session` so a command line can never become a session draft, and from the palette's `Esc`/`Enter` handlers in `connection/chat.rs`). Shortcut labels come from the logical keymap in `state/keymap.rs` (below). |
| `state/keymap.rs` | The Chat page's LOGICAL keymap: one `Alt+<key>` shortcut per command (`Alt+M` model, `Alt+R` reasoning, `Alt+Enter` continue, `Alt+Up` undo, `Alt+Down` redo, `Alt+S` session, `Alt+A` account, `Alt+Q` quit), lowered from a crossterm `KeyEvent` by `chord_from_event`. Every app command lives on an `Alt+` chord because emacs/readline owns the `Ctrl+<letter>` space (the editing kernel's movement/kill bindings — see `state/input.rs` below), and `Alt+letter` is reported distinctly by every terminal — so there is NO terminal-specific rebinding and the old `Ctrl+M`/`Ctrl+O` duality (and `resolve_chord`) is gone. `binding_for` is the single-source-of-truth lookup Chat-page dispatch routes through (so a shortcut and its typed `/command` spelling run one path), and `shortcut_label_for` feeds the palette's right-aligned labels; `help` is deliberately absent (it toggles the overlay, not a catalog command) and `quit` is listed for display only (it stays the global pre-dispatch in `connection/mod.rs`). |
| `state/input.rs` | The shared text-editing kernel (`InputBuffer`) used by every text field (chat input, model-selector filter, wizard filter/slug, credential modal): grapheme-aware insert/delete and cursor movement, the visual-line wrapping cache, click→byte-offset mapping, and the readline/emacs key handling (`handle_key`). Beyond the app's own `Ctrl+W`/`Ctrl+U`/`Ctrl+Backspace`/`Ctrl+Delete`/`Ctrl+Left-Right`/`Home`/`End`/`Ctrl+Home-End`, it implements the shell's readline movement and kill chords: `Ctrl+A`/`Ctrl+E` (beginning/end of the logical line — `cursor_home_line`/`cursor_end_line`), `Ctrl+B`/`Ctrl+F` (one char), `Alt+B`/`Alt+F` (one word), `Ctrl+D` (delete char), `Ctrl+H` (delete char back — needed because a kitty terminal sends `Char('h')+CONTROL` while a legacy one sends Backspace), `Ctrl+K` (kill to the end of the logical line — `kill_line`, never consuming the `\n`), `Ctrl+T` (transpose graphemes — `transpose_chars`), `Alt+D` (kill word forward), and `Alt+Backspace` (kill word back). Pure cursor moves leave the buffer's `generation` untouched (so a recalled history entry stays attached); the mutating chords bump it (detaching the entry — see `App::edit_input`). `Ctrl+P`/`Ctrl+N` (readline history) are driven one layer up in `connection/chat.rs` because they need `App` state, and reuse the `↑`/`↓` helpers. |
| `render/command_palette.rs` | The inline command-palette overlay renderer: when `App::command_palette_active()`, it draws the filtered command list (matched prefix emphasized, summary, and the right-aligned per-command shortcut label from `shortcut_label_for`) in a floating box anchored to the command input box's top edge (shared `input_box_rect` geometry) and `Clear`-ed, so it never covers the input box or the status bar below it — only the help/history rows above are overlapped. The list is windowed to `COMMAND_PALETTE_MAX_ROWS` via the shared `picker_window` (with a `+N more` tail row) and the highlighted row is marked `>`. A no-op when there are no matches, no vertical room (`input.y == 0`), or a zero-area box. |
...
| Group | Tools |
|---|---|
| **Core** | `list_sessions`, `get_session`, `load_skill`, `set_session_title`, `set_working_dir`, `load_tools`, `unload_tools`, `read_file`, `write_file`, `edit_file`, `list_files`, `delete_files`, `line_count`, `random` (integers, floats, booleans, bytes, UUID v4 — with optional seed), `get_current_time` (Unix millisecond timestamp), `pdf_classify` (PDF type/confidence/OCR pages), `pdf_to_markdown` (PDF → Markdown, optional pages + compact), `retrieve_webpage` (render a URL in a local headless Chromium/Chrome — `http`/`https`/`file` — content / text / screenshot (PNG, inline or to `output_path`) / pdf (to `output_path`); opt-in `webgl` for new-headless + SwiftShader WebGL rendering) |
...
The vision bytes never reach clients: `turn_for_client` strips `ToolResultRecord.image` from the
client-facing turn. Displayed images (`displayed_images`), produced by `display_image`,
`generate_image`, and `retrieve_webpage` screenshots, persist in the same
`session_attachments` table but are served to clients **on demand**: the session-scoped
snapshots carry only `ImageMetadata`, and a client fetches an image's bytes when it needs to
render it (`ClientMessage::GetImage` → `DaemonMessage::Image`, keyed by `(session_id,
turn_id, image_index)`; the CONNECTION THREAD reads a single `d{i}` slot via
`read_display_image` against its own shared `Arc<redb::Database>` handle — no
command-loop round-trip).
...
| MCP (choreo-mcp) | Server spawn, tool discovery, echo tool call/response | `choreo-mcp/tests/it/mcp_integration.rs` |
| MCP (daemon) | McpManager + ToolRegistry integration, dynamic group registration, tool execution | `choreo-daemon/tests/it/mcp_integration.rs` |
