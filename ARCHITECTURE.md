...
**The empty-message fallback** closes the remaining hole in that injection: a DeepSeek/Kimi turn recorded as *reasoning-only* (empty content, no tool calls — e.g. a response that streamed only `reasoning_content`) would serialize as `content: ""` + the injected empty `reasoning_content`, i.e. a wholly empty assistant message that the provider rejects with "the message ... with role 'assistant' must not be empty". The single `include_reasoning_artifact()` helper (used by the builder, the precondition guard, and `session_inspect`) forces such a turn's **same-model** artifact in even though ToolLoop alone would skip it (no tool involvement): the artifact's real reasoning text is the only payload that keeps the wire message non-empty. A foreign-model artifact (mid-session switch) or a missing artifact leaves the message unfixable — the guard flags it as a "must not be empty" risk instead of letting the provider fail silently; this is exactly the opencode-go deepseek→kimi shape (an empty assistant turn in history 400s the very next Continue).
