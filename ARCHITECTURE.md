...
- **Sink.** `choreo-tui`'s `sanitize_for_terminal` filter runs over every tool-result
  ...
  would let hostile content overwrite its own rendered line; a CRLF pair is a
  normal line ending and is folded through), U+2028/U+2029, and the Cf spoofing class (via the same shared
