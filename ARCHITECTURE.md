...
| `daemon/keystore.rs` | The keystore command handlers, lifted out of `daemon.rs` (same child-module pattern as `daemon/subscriber_handlers.rs`): the `Unlock`/`BindKeystore`/`Lock`/`AddCredential` handlers enforce the verify-only vs bind split (`handle_unlock_inner` / `handle_bind_keystore_inner` — the latter is the ONLY adoption path, TOFU on an unbound keystore) and run the shared `unlock_tail` (bulk-decrypt, load accounts, resolve providers, clear `locked`). The cached `keystore_bound` flag is flipped `true` here (in `bind_keystore`) — the single-writer adoption path — backing the three-state keystore status: `current_keystore_message`/`broadcast_keystore_state` push `DaemonMessage::Keystore { state }` (`Unbound` when unbound, else `Locked`/`Unlocked`) to each activity subscriber at subscribe time and on every transition. These `DaemonCommand`s carry the acting client's `SubscriberSink` so the command loop can enqueue the targeted reply (`Unlocked`/`Bound`/`KeystoreUnbound`/`LockedError`/`CredentialAdded`) via `send_targeted` BEFORE any lock-state transition broadcast — the ORDERING INVARIANT the client's key-recording correctness keys on (pinned by `unix_targeted_reply_precedes_lock_state_broadcast`); a `/lock` against an UNBOUND daemon broadcasts no transition (already-`Unbound` for the subscribers, so the broadcast is redundant chatter). The pure keystore SEMANTICS (binding adoption/verification, crypto, blob storage) stay in the `choreo-keystore` crate — this module is the daemon's ORCHESTRATION of them. |
