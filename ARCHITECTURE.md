...
| `sessions.rs` | `SessionState` (split into `SessionConfig` for persisted fields + runtime state) — now also owning this session's `SocketRegistry` and a LAZILY-built provider client (`provider: Option<InferenceProvider>`; sessions can exist while the keystore is locked, so the client is built on the session thread at the first request against the session's registry, and is dropped-on-`/lock` via `SessionCommand::DropProvider` to rebuild from fresh credentials), `RequestContext` dependency bundle, `SessionCommand` enum and its handler functions. Each session has a control thread running `session_main()`; request work runs on separate worker threads via `run_request_worker()`. Sessions form a tree (parent → child sub-sessions), each with an optional working directory. |
