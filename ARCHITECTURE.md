...
| `state/command_palette.rs` | The inline command palette (Chat page) plus the **command-entry mode** it drives: pressing `/` on an EMPTY prompt enters command mode (the `/` is a trigger and never enters the buffer), the shared composer buffer then holds the bare command line (no leading slash), the palette is shown while `command_mode` is set, and `Enter` RUNS the command (while `Esc` returns to the prompt). `command_palette_enter_line` resolves the line Enter runs: a first token that already names a command EXACTLY runs verbatim (arguments preserved), while an empty or still-partial token adopts the HIGHLIGHTED palette row (keeping any argument tail), so Enter alone runs the selected command with no preceding `Tab`; with no highlighted match the line runs verbatim so the parser still reports an unknown command. Owns the palette's transient selection state (`CommandPaletteState` — highlight index + scroll hint), the `enter_command_mode`/`exit_command_mode` transitions (both clear the buffer and reset the palette; `exit_command_mode` is a guarded no-op outside command mode, so a real prompt draft survives page changes / session switches, and both are also invoked from `App::set_page` / `reset_for_session_switch` / `attach_to_session` so a command line can never become a session draft), first-token-filtered `match_commands` results, wrap-around `↑`/`↓` movement, `Tab` completion (rewrites the buffer to `"<name> "` — no slash — never submitting; `Tab` remains the pure name-completion that keeps the user in the palette), and `command_palette_active` (= `command_mode && !model_selector.is_open()`, TRUE even with zero matches so a "no matches" palette still shows and `Enter` still submits the line). It also owns the LOGICAL shortcut table (`SHORTCUTS`, one binding per command) and its resolution: `chord_from_event` lowers a crossterm `KeyEvent` to a logical `Chord`, `resolve_chord` applies the terminal-specific rebinding (on a legacy terminal the logical `Ctrl+M` resolves to `Ctrl+O`, since byte 0x0D makes `Ctrl+M` indistinguishable from Enter), and `binding_for` / `shortcut_label_for` are the single-source-of-truth lookups that Chat-page dispatch and the palette's right-aligned labels both use — so a shortcut and its typed-command spelling run one command path. |
